    ...
    <div className={`absolute z-50 flex flex-col items-start transition-all duration-300 ${isFullscreen ? 'bottom-6 left-6' : '-bottom-14 left-0'}`}>
      ...
      {!isOpen && (
        <motion.button
          ...
          className="px-4 py-3 bg-gray-800/90 hover:bg-cyan-600/90 text-cyan-400 hover:text-white rounded-full backdrop-blur-md border border-gray-600/50 shadow-[0_0_15px_rgba(34,211,238,0.3)] transition-all flex items-center justify-center gap-2 group"
          title="開啟快速導覽"
        >
          <Compass className="w-5 h-5 group-hover:rotate-90 transition-transform duration-500" />
          <span className="text-sm font-medium tracking-wide">快速導覽索引</span>
        </motion.button>
      )}
    </div>
  );
